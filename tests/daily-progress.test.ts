...
import { createEmptyActiveDay, getTodayTotal, mergeActiveDay, normalizeActiveDay, recordFileObservation, renameFileProgress } from "../src/daily-progress";
...
test("net change is floored at zero", () => {
	...
	activeDay = recordFileObservation(activeDay, "2026-04-04", "note.md", 100, 1);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "note.md", 80, 2);
	assert.equal(getTodayTotal(activeDay), 0);
});

test("per-file totals are aggregated", () => {
	let activeDay = createEmptyActiveDay("2026-04-04");
	activeDay = recordFileObservation(activeDay, "2026-04-04", "a.md", 10, 1);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "a.md", 25, 2);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "b.md", 5, 3);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "b.md", 9, 4);
	assert.equal(getTodayTotal(activeDay), 19);
});

test("rename keeps progress without duplication", () => {
	let activeDay = createEmptyActiveDay("2026-04-04");
	activeDay = recordFileObservation(activeDay, "2026-04-04", "old.md", 20, 1);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "old.md", 30, 2);
...
test("rename moves progress and a recreated old path starts fresh", () => {
	...
	activeDay = recordFileObservation(activeDay, "2026-04-04", "old.md", 30, 2);
	activeDay = renameFileProgress(activeDay, "old.md", "new.md");
	activeDay = recordFileObservation(activeDay, "2026-04-04", "old.md", 50, 3);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "old.md", 70, 4);
...
test("active-day merge keeps earliest baseline and newest latest count", () => {
	...
	local = recordFileObservation(local, "2026-04-04", "note.md", 100, 10);
	local = recordFileObservation(local, "2026-04-04", "note.md", 120, 20);
	let incoming = createEmptyActiveDay("2026-04-04");
	incoming = recordFileObservation(incoming, "2026-04-04", "note.md", 90, 5);
	incoming = recordFileObservation(incoming, "2026-04-04", "note.md", 140, 30);
...
test("first observation of a pre-filled file starts with zero net new words", () => {
	let activeDay = createEmptyActiveDay("2026-04-04");
	activeDay = recordFileObservation(activeDay, "2026-04-04", "prefilled.md", 2000, 1);
...
test("empty file can still grow from zero to counted words", () => {
	let activeDay = createEmptyActiveDay("2026-04-04");
	activeDay = recordFileObservation(activeDay, "2026-04-04", "draft.md", 0, 1);
	activeDay = recordFileObservation(activeDay, "2026-04-04", "draft.md", 2000, 2);
